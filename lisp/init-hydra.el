...
(pretty-hydra-define-e hydra-toggles
  ...
  ("Basic"
   ...
   (("h l" global-hl-line-mode "line" :toggle t)
    ...
    ("h n" display-line-numbers-mode "line number" :toggle t))
   "Ui"
   (("d" +lizqwer/toggle-dark-theme "dark theme" :toggle (cl-find user/night-theme custom-enabled-themes))
    ...
    ("r f" a-random-theme "rand favorite theme")
    ("r d" random-theme-dark "rand dark theme")
    ("r l" random-theme-light "rand light theme")
    ...
    (when (and packages
      ...
      (dolist (pkg packages)
        (message "[%d/%d] %s %s..."
                 ...
                 (length packages) prompt pkg)
        (funcall command pkg))
      (message "%s finished for: %s" prompt (string-join packages ", ")))))
